...
import {ProtocolError} from 'appium-base-driver/build/lib/protocol/errors';
...
const ATOM_WAIT_TIMEOUT = 5 * 60000; // eslint-disable-line no-unused-vars
...
extensions.waitForAtom = async function waitForAtom (promise) {
  ...
  if (timedOut) {
    for (let i = 0; i < 10; i++) {
      ...
      await this.checkForAndHandleAlert(true);
      await B.delay(500);
    }
  }
  res = await promise;
...
extensions.checkForAndHandleAlert = async function checkForAndHandleAlert (isRetroactive) {
  let alertText;
  try {
    alertText = await this.proxyCommand('/alert/text', 'GET');
  } catch (ign) {}
  if (util.hasValue(alertText)) {
    let message = 'Alert detected ' + (isRetroactive ? 'after action had already started.' : 'before action performed');
    log.debug(`Found an alert with text '${alertText}'`);

    const unhandledPromptBehavior = this.settings.getSettings();
    if (unhandledPromptBehavior.includes('accept')) {
      log.debug('Accepting the alert as requested by unhandledPromptBehavior capability');
      try {
        await this.proxyCommand('/alert/accept', 'POST', {});
      } catch (err) {
        log.warn(`Did not accept the alert. Original error: ${err.message}`);
      }
    } else if (unhandledPromptBehavior.includes('dismiss')) {
      log.debug('Dismissing the alert as requested by unhandledPromptBehavior capability');
      try {
    ...
    if (unhandledPromptBehavior.includes('notify') || unhandledPromptBehavior === 'ignore') {
      throw new UnexpectedAlertOpenError(message, alertText);
    }
  }
};
